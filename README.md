...
- **Detección de facturas ya cargadas.** Antes de guardar, el sistema busca si ya existe una factura del mismo proveedor con el mismo N° de comprobante (ignorando espacios y guiones), o del mismo proveedor con la misma fecha y el mismo monto. Si la encuentra, pide confirmación explícita antes de duplicarla.
- **Escaneos incompletos o con foto de mala calidad.** La foto se le manda a la IA en alta resolución (la copia que se archiva en la base sigue siendo liviana), y el escaneo devuelve además una evaluación de la calidad de la imagen y la lista de campos que no pudo leer. Si el proveedor no quedó identificado, el panel de escaneo lo pide de entrada (por nombre o CUIT) para poder seguir con el reconocimiento automático de los ítems; si la foto salió mal, avisa qué campos conviene verificar a mano en vez de dar por buenos datos dudosos.
